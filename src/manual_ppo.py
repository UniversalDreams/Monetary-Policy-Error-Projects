...
import config
from fed_env import FedEnvBase, StateKeyedLLMWrapper
...
def main():
    parser = argparse.ArgumentParser(description="Manual PPO training: baseline vs LLM")
    ...
    parser.add_argument("--condition", type=str, default="both", choices=["base", "offline", "both"])
    ...
    if args.condition in ("offline", "both"):
        ...
        llm_envs = DummyVecEnv(
            [lambda: StateKeyedLLMWrapper(FedEnvBase(llm_dim=config.LLM_DIM), db_path=args.db) for _ in
             range(config.N_ENVS)])
